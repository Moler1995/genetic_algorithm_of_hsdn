    ...
    def aimFunc(self, pop):
        ...
        for one_pop in pop_values:
            ...
            for i in range(self.node_size):
                ...
                shortest_path = gu.dijkstra_alg(filled_weight_list, i)
                # 先将每个节点堪称传统节点，以每个顶点为目标节点，构建有向无环图
                legacy_node_dag = gu.build_dag(filled_weight_list, i, shortest_path)
                # 针对每一个顶点的有向无环图查找sdn节点，增加可用链路并验证环路
                dag, sorted_nodes = gu.add_links(filled_weight_list, legacy_node_dag, i, one_pop[:self.sdn_node_count])
                near_optimal_bandwidth_used = nosr.execute(dag, sorted_nodes, self.traffic,
                                                           self.band_width, self.band_width)
